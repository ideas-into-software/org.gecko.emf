...
public class MongoClientProviderTest {
	...
	public void testCreateMongoClientProvider(
		...
		p.put(MongoConstants.CLIENT_PROP_CLIENT_MONGO_IDENT, clientId);
//		p.put("credential.type", "SCRAM_SHA_1");
		...
		}).untilServiceEventRegistered(MongoDatabase.class).assertWithTimeoutThat(1000).hasServiceEventsThat()
				.hasSize(2);
...
//		Assertions.assertThat().isEqualTo("");
		...
		MonitoringAssertion.executeAndObserve(() -> {
			...
			try {
				Configuration c = cm.getConfiguration(MongoConstants.PID_MONGO_CLIENT + "~base");
	...
	void testDatabase(@InjectService(cardinality = 0) ServiceAware<MongoClient> serviceAwareClient,
		...
		MonitoringAssertion.executeAndObserve(() -> {
			...
			try {
				Configuration c = cm.getFactoryConfiguration(MongoConstants.PID_MONGO_CLIENT, "base", "?");
				c.update(Dictionaries.dictionaryOf(MongoConstants.CLIENT_PROP_CLIENT_MONGO_IDENT, "aaa",
		...
		MonitoringAssertion.executeAndObserve(() -> {
			try {
				...
				c.update(Dictionaries.dictionaryOf(MongoConstants.DB_PROP_DATABASE_INTERNAL_NAME, internalName,
						MongoConstants.DB_PROP_DATABASE_MUST_EXIST, true));

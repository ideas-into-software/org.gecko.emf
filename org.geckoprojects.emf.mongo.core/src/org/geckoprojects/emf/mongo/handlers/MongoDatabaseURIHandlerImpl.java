...
public class MongoDatabaseURIHandlerImpl extends URIHandlerImpl
{
	 ...
	 */
	public MongoDatabaseURIHandlerImpl(MongoDatabase mongoDatabase,Map<String,Object> dbProps, InputStreamFactory inputStreamFactory, OutputStreamFactory outputStreamFactory) {
//		verifyAndSet(mongoDatabase, dbProps);
	 ...
	 * 
	 * @param mongoDatabase to be verifyed
	 * @param dbProps 
	 * @throws IOException if the URI is malformed or the collection could not otherwise be resolved
	 */
	private void verifyAndSet(MongoDatabase mongoDatabase, Map<String, Object> dbProps) throws IOException {
		// We assume that the URI path has the form /database/collection/{id} making the
		// collection segment # 1.
		
		Object oUri = dbProps.get("db.uri");
		if (oUri == null) {

			throw new IOException("The URI is not given");

		}
		URI uri = URI.createURI(oUri.toString());

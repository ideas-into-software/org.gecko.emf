...
public abstract class AbstractRuntimeMonitoringAssert<SELF extends AbstractRuntimeMonitoringAssert<SELF, ACTUAL>, ACTUAL extends EventRecording>
	...
	private SELF hasServiceEventsInOrder(boolean exact, List<Condition<ServiceEvent>> conditions) {
		...
		for (ServiceEvent se : list) {
			sb = sb.append("ServiceEvent " + eventCounter++ + ":").append(System.lineSeparator()).append(" type: ")
					...
					.append(se.getServiceReference() == null ? "[]" : se.getServiceReference().getProperties())
	...
	private String doMessage(int expectedCount, long wasCount, String compareText, Map<ServiceEvent, String> result) {
		...
		eventsPassed.forEach(e -> {
			...
			sb.append(seToString(se)).append(System.lineSeparator()).append(System.lineSeparator());
		...
		eventsNotPassed.forEach(e -> {
			ServiceEvent se = e.getKey();
			sb.append(seToString(se)).append(System.lineSeparator());
	...
	private String seToString(ServiceEvent se) {
		return new StringBuilder().append("ServiceEvent: type <")
				...
				.append(Arrays.toString((String[]) se.getServiceReference().getProperties().get("objectClass")))
				.append(System.lineSeparator()).append("SR-Properties: ")
				.append(se.getServiceReference().getProperties()).toString();
